...
export class FormsController {
  ...
  async getMyApplications(@Req() req: Request) {
    try {
      ...
      const result = await this.supabaseService.getUserApplications(user.id);
      return { success: true, data: result };
    } catch (error: any) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  @Get('my-donations')
  @UseGuards(SupabaseAuthGuard)
  async getMyDonations(@Req() req: Request) {
    try {
      const user = (req as any).user;
      const result = await this.supabaseService.getUserDonations(user.id);
      return { success: true, data: result };
    } catch (error: any) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
